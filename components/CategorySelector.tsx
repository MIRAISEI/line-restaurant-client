    ...
    return (
        <div className="flex overflow-x-auto pb-4 gap-4 no-scrollbar -mx-4 px-4 md:mx-0 md:px-0 scroll-smooth">
            {categories.map((category) => {
                ...
                const jsonName = t(category.name as any);
                const hasJsonTranslation = jsonName && jsonName !== category.name;

                const displayName = dbName || (hasJsonTranslation ? jsonName : category.name);
